...
var adocAbbreviationsPreprocess4Tex = module.exports = function adocAbbreviationsPreprocess4Tex(the_content){

	logger.debug(`\n\n Begin adocAbbreviationsPreprocess4Tex`);			
	...
	while((the_case=EXT_PATTERN_REG.exec(the_content))!==null){//Find one :xxx-abrev: we need to process all xxx for that
		...
		// Create tex_glossary.tex 
		acro_entry=the_case[1];
		acro_definition = the_case[2];
		acro_display = the_case[3];
		acro_definition = acro_definition.replace(/&amp;/g,'\&').replace(/\&/g,`\\&`).trim();
		acro_display = acro_display.replace(/&amp;/g,'\&').replace(/\&/g,`\\&`).trim();

		the_tex_glossary_content = 
		`${the_tex_glossary_content}\\ifglsentryexists{${acro_entry}}{%
		}{%
		  \\newacronym{${acro_entry}}{${acro_display}}{${acro_definition}}%
		}
		\n
		`;
        ...
        //NOTE: tex does not need first replacement
	...
	logger.debug(`I am going to write glossaries to ${the_tex_glossary_file_path}: \n${the_tex_glossary_content}`);
	fs.writeFileSync(the_tex_glossary_file_path, the_tex_glossary_content);			
	return the_result;
};
