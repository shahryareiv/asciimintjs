...
<!-- Roman Numbers -->
<xsl:template match="phrase[contains(concat(' ', normalize-space(@role), ' '), ' romannumup ')]  | phrase[contains(concat(' ', normalize-space(@role), ' '), ' amRNU ')]">
  <xsl:text>\amRomanNumUp{</xsl:text>
    <xsl:apply-templates/>
  <xsl:text>}</xsl:text>
</xsl:template>
<xsl:template match="phrase[contains(concat(' ', normalize-space(@role), ' '), ' romannumlow ')] | phrase[contains(concat(' ', normalize-space(@role), ' '), ' amRNL ')]">
...
<!-- This both covers marginfigurenotes with inline image(s) and  marginfigurenotes with title plus block images -->
 ...
 <xsl:template match="*[contains(concat(' ', normalize-space(@role), ' '), ' amTwoPageFigure ')]">
  <xsl:text>\amTwoPageFigure{t}{l}{</xsl:text><!-- Shahryar -->
      <xsl:value-of select="./mediaobject/imageobject/imagedata/@fileref" />  
  <xsl:text>}{</xsl:text><!-- Shahryar -->
  <xsl:apply-templates select="title"/>      
  <xsl:text>}&#10;</xsl:text><!-- Shahryar -->
...
<xsl:template match="figure">
  ...
  <xsl:variable name="environ">
    <xsl:choose>
      ...
      <xsl:when test="@role ='amFullFigure'">figure*</xsl:when>
